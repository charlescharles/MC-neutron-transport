...
def process_counts():
    ...
    try:
        with open('processed_dim_variations_dist100', 'r') as f:
            try:
                last = list(f)[-1]
    ...
    files = os.listdir('dim_variations_rad1.5_dist100')
    for rad in radii:
        print('rad:{0}'.format(rad))
        template = bin('templates/dim_template_dist100', rad)
        n = len(template[0])
        for i in range(n):
            for j in range(n):
                template[i][j] /= 100
        # write and close one radius at a time.
        with open('processed_dim_variations_dist100', 'a') as f_out:
            for fname in files:
                print(fname)
                deviation = r.search(fname).group(1)
                test = bin('dim_variations_rad1.5_dist100/' + fname, rad)
                dof = n**2 - 1
